...
def get_saliency_rbd(img):
	...
	img_lab = img_as_float(skimage.color.rgb2lab(img_rgb))
	print("img rgb shape", np.shape(img_rgb))
	img_gray = img_as_float(skimage.color.rgb2gray(img_rgb))
        
	
	
        
	segments_slic = slic(img, n_segments=250, compactness=10, sigma=1, enforce_connectivity=False)
	...
	sal = np.zeros((img_disp1.shape[0],img_disp1.shape[1],1))

	sal = img_disp2
	sal_max = np.max(sal)
	sal_min = np.min(sal)
	#print("sal max", sal_max)
	#print("sal min", sal_min)
	print("sal", sal)
...
for file in HS_files:
    ...
    mat_dict = scio.loadmat(input_img)
        
    for key in mat_dict:
        if type(mat_dict[key]) is np.ndarray:
            image_data = mat_dict[key]  # type: np.ndarray
    print("image data shape is", np.shape(image_data))
    #print("type is",type(mat_dict[key]))
    cols = image_data.shape[1]
    rows = image_data.shape[0]
    bands = image_data.shape[2]
    image_data1 = image_data.reshape(768, 1024,3)
    salmap = get_saliency_rbd(image_data1).astype('uint8')


    file = file.rsplit('.', 1)
    print("file name catentaed", file)
    save_sal_name = sal_result_path + '/' + file[0] + '.png'
    cv2.imwrite(save_sal_name, salmap)
    num = num+1    
t = (time.time() - start_time)
f = open("time_taken.txt",'wt');
f.write(str(t));
f.close()
print("finished") 
